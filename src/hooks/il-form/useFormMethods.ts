 ...
 * @LastEditTime: 2021-09-17 18:45:00
 * @LastEditors: mjqin
 */
...
export const useFormMethods = () => {
  ...
  const validate = (callback: validateCallback) => {
    (ctx?.$refs.formRefs as typeof ElForm).clearValidate(callback);
  };

  const validateField = (props: string[] | string, callback: validateFieldCallback) => {
    (ctx?.$refs.formRefs as typeof ElForm).clearValidate(props, callback);
  };
  ...
  const scrollToField = (prop: string) => {
    (ctx?.$refs.formRefs as typeof ElForm).clearValidate(prop);
  };

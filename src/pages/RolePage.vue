...
<script>
import AddEditRoleDialog from "../components/role/AddEditRoleDialog.vue";
...
export default {
  name: "RolePage",
  components: { DelDialog, AddEditRoleDialog },
  setup() {
    ...
    const getRoleList = async () => {
      ...
      try {
        ...
        const { count, rows } = await ctx.$api.getRoleList(params);
        roleList.value = rows;
        pager.total = count;
      } catch (error) {
        ctx.$message.error(error.msg || error);
    ...
    const getPermissionList = async () => {
      try {
        ...
        const { rows } = await ctx.$api.getPermissionList(params);
    ...
    const deleteRole = async (ids) => {
      ...
      try {
        await ctx.$api.deleteRole({ id: ids });
